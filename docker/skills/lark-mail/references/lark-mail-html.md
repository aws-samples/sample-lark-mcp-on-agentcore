...
> 本文档定义邮件写信场景下的 HTML / CSS / URL 写法、LarkSuite mail-editor 原生格式、可复制片段、场景模板。（认证由 MCP server 自动处理。）

**CRITICAL 邮件是重要的对外交流渠道，请你保证书写语言凝练扼要**
**CRITICAL 电子邮件的 HTML 不是 Web 开发的 HTML，请你务必遵守本文档中提及的常用邮件格式书写规范**
**CRITICAL 请务必使用写信工具来进行邮件内容编辑（`lark_mail_send` / `lark_mail_draft_create` / `lark_mail_reply` / `lark_mail_reply_all` / `lark_mail_forward`）或 `lark_mail_draft_edit` 的 body op，严禁自行拼接 EML**

你可以参考 **官方模板库**（仓库 `assets/templates/` 目录）— 提供部分场景模板，可供参考；用 `lark_get_skill(domain="mail", section="assets/templates/<name>.html")` 按名取用某个模板

> 请注意，邮件内容编辑相关的写信工具内置 HTML lint 工具，处于安全考虑和格式适配，你输入的 HTML 可能会被自动调整
...
仓库 `assets/templates/` 目录内预制了若干场景模板，按 LarkSuite mail-editor 原生格式写好。用 `lark_get_skill(domain="mail", section="assets/templates/<name>.html")` 取用某个模板。**注意：模板是静态 HTML，没有变量替换能力，AI 需要手工把模板里的样例文本替换成本次邮件的真实内容。**
...
1. **判断是否能用模板** — 看用户当前要写的邮件类型（周报 / 调研 / 简历 / 资讯 / ...）能否对上上表里的某个模板文件；不匹配就跳过模板，直接按写法规范从零写。
2. **取用整个 HTML** — 用 `lark_get_skill(domain="mail", section="assets/templates/<name>.html")` 完整读取选定的模板文件，理解骨架（章节标题 / 列表层级 / 占位文本 / mention chip / 段落顺序）。
...
5. **拿到草稿链接给用户复核** — 写信工具返回 `reference` 字段（草稿打开链接），把它给用户在飞书邮箱 UI 里打开核对，再决定下一步发送 / 编辑。

## 写信工具的 lint 返回值

写信链路（`lark_mail_send` / `lark_mail_draft_create` / `lark_mail_reply` / `lark_mail_reply_all` / `lark_mail_forward` / `lark_mail_draft_edit` body op）调用 `emlbuilder` 之前会强制 lint 净化 HTML，但 **默认返回不携带任何 lint 字段**（既无 `*_count` 也无 finding 数组），返回保持小巧供 AI 消费。每个写信工具默认返回的字段集合：

| 字段 | 出现条件 | 说明 |
|------|---------|------|
| `compose_hint` | 6 个写信工具默认都附 | 固定英文文案，提示 AI / 用户在组合 HTML 前阅读本文 |
| `draft_edit_hint` | **仅** `lark_mail_draft_create` 默认附（其他 5 个写信工具不附） | 固定英文文案，提示拿到 `draft_id` 后改稿走 `lark_mail_draft_edit(draft_id="<id>")` 而不是重跑 `lark_mail_draft_create` 产生重复草稿 |
| `draft_id` / `message_id` | OAPI 写入成功后写回 | `lark_mail_draft_create` / `lark_mail_draft_edit` 返回 `draft_id`；`lark_mail_send` / `lark_mail_reply` / `lark_mail_reply_all` / `lark_mail_forward` 返回 `message_id` |
...
如果只是想预览 lint 会怎么改 HTML，建议直接用 `lark_mail_lint_html` 工具（详见 `lark_get_skill(domain="mail", section="lint-html")`）——它本来就返回完整 `warnings[]` / `errors[]` + `cleaned_html`，比写信链路 `show_lint_details=true` 更清晰。

## 相关文档

- `lark_mail_lint_html` 用法：`lark_get_skill(domain="mail", section="lint-html")`
- 写信工具: `lark_mail_send` / `lark_mail_draft_create` / `lark_mail_reply` / `lark_mail_reply_all` / `lark_mail_forward` / `lark_mail_draft_edit`（各自详见 `lark_get_skill(domain="mail", section="send")` 等）
