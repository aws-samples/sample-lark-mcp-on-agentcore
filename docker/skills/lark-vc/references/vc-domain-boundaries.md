...
- **lark-calendar** 负责日历与日程管理，包括创建、查询、修改、删除日程等操作。
- **日程与会议的关系**：日程可以用于提前预约会议，确定会议时间、参与人、会议室、会议主题等信息。日程上可以关联飞书/Lark 视频会议。
- **并非所有会议都通过日程发起**：即时会议不经过日程预约，直接创建。因此，仅查询日程数据无法覆盖所有会议，搜索历史会议应优先使用 `lark_vc_search`。
- **日程上的用户会议纪要**：用户可以在日程上绑定自己的会议纪要文档（MeetingNotes），用于手动记录会议相关信息。该文档与 AI 生成的智能纪要（`note_doc_token`）是不同的文档，相互独立。

> **路由规则**：查询过去已结束的会议 → lark-vc；查询未来日程/待开的会 → lark-calendar；查询"今天有哪些会议" → 两者结合（`lark_vc_search` 查已结束 + lark-calendar 查未开始）。

## VC 域

- **lark-vc** 负责视频会议管理，包括搜索历史会议、查询会议产物（智能纪要、逐字稿、妙记等）、查询参会人快照等操作。
...
智能纪要的逐字稿（`normal` 纪要的 `verbatim_doc_token` 文档、`unified` 纪要的 lark-note 纪要逐字稿输出）和妙记的文字记录（Transcript）都记录了用户原始对话内容，格式一致：
...
详细用法请调用 `lark_get_skill(domain="vc", section="search")`。
...
详细用法请调用 `lark_get_skill(domain="doc")` 与 `lark_get_skill(domain="note")`。
...
- Doc 是会议产物的载体，智能纪要和逐字稿都以飞书文档形式沉淀，需通过 Doc 域工具读取。
