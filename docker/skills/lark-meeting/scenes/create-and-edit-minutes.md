...
妙记 AI 待办不是飞书任务。上下文包含妙记 URL / `minute_token` 并要求修改妙记待办时，禁止改走飞书任务域（`lark_get_skill(domain="task")`）；用户同时指定了负责人（包括"负责人是我"）也不改变归属。
...
- 用户说"负责人是我"时，先用 `lark_contact_get_user()` 取当前登录用户的真实姓名再拼接；取不到就不写任何 `@` 提及，不要保留字面的 `@我`。
- 姓名解析只影响追加的 `@` 文本，绝不能阻塞或取消待办创建；不要为处理负责人改走飞书任务域或做进一步通讯录搜索。
- 不要用"以你的身份创建即归属于你"代替真正的 `@` 文本拼接；调用身份和负责人是两件不相关的事。
- 回复只陈述结果（妙记、待办内容、负责人、完成状态），不要解释接口字段限制，也不要建议改用飞书任务来"明确负责人"。

完整规则和示例见 `lark_get_skill(domain="meeting", section="lark-minutes-todo")` 的「负责人 / `@` 提及」。
...
`lark_minutes_upload`、`lark_minutes_summary`、`lark_minutes_todo` 和 `lark_minutes_word_replace` 都可能返回 `quota_exceeded`，表示 ASR/AI 额度已耗尽。`lark_minutes_upload` 是额度不足以转写这个音视频，妙记根本没有创建；其余三个是该妙记生成时额度就已用尽、AI 产物未完整生成，写操作无法落库。

请用户去妙记详情页查看额度详细信息，不要重试：无法通过工具补充或提升额度，重试同样的请求不会成功。这不是权限问题，也不要误走 `lark_minutes_apply_permission` 或重新走授权流程。

## 确认修改结果

修改前只读取目标相关字段，修改后用 `lark_minutes_detail` 或对应读取工具回读。工具自身已逐项返回写入结果时不再回读，例如 `lark_minutes_word_replace` 的 Succeeded / Failed 关键词。批量或多步修改逐项报告写前值、写后结果和失败原因；部分成功时不要回滚已成功项，除非工具明确承诺原子回滚。
