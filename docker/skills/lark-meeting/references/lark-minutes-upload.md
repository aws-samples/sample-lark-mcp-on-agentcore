...
本接口不直接处理本地文件的上传，必须先使用 `lark_drive_upload` 将文件上传到云空间（云盘/云存储）获取 `file_token`，然后再调用本接口。上传细节见 `lark_get_skill(domain="drive", section="upload")`。

### 2. 异步生成

API 会立即返回 `minute_url`，但妙记可能仍在异步生成中。`lark_minutes_upload` 不返回处理状态，调用成功只表示异步创建请求已提交；只有后续执行 `lark_minutes_detail` 并确认就绪，才能声称妙记产物已生成或可用。上传与后续产物获取由 `lark_get_skill(domain="meeting", section="scenes/create-and-edit-minutes")` 编排；MCP server 未暴露 `lark_minutes_detail` 的等待参数，上传后立即查询产物时按返回状态自行轮询。
