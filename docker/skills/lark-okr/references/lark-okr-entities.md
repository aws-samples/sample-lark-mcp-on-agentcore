...
> **SHORTCUT：** `lark_okr_cycle_list` (参见 `lark_get_skill(domain="okr", section="cycle-list")`) 获取用户的周期列表，可按时间筛选
>
> **API：** `cycles.list`
...
| `content`     | `ContentBlock` | 否  | 目标内容（富文本），见 `lark_get_skill(domain="okr", section="contentblock")` |
| `score`       | `number`       | 否  | 目标分数，范围 [0, 1]，支持一位小数                                   |
| `notes`       | `ContentBlock` | 否  | 目标备注（富文本），见 `lark_get_skill(domain="okr", section="contentblock")` |
| `weight`      | `number`       | 否  | 目标权重，范围 [0, 1]，支持三位小数                                   |
| `deadline`    | `string`       | 否  | 截止时间，毫秒时间戳，shortcut 会将其解析为日期时间                          |
| `category_id` | `string`       | 否  | 所属分类 ID                                                 |

> **SHORTCUT：**
> - `lark_okr_cycle_detail` (参见 `lark_get_skill(domain="okr", section="cycle-detail")`) 获取某个用户周期下的全部目标和关键结果。时间相关的字段会以日期时间格式解析
>
> **API：**
...
| `position`     | `integer`      | 是  | 排序序号，从 1 开始，范围 [1, 100]                                   |
| `content`      | `ContentBlock` | 否  | 关键结果内容（富文本），见 `lark_get_skill(domain="okr", section="contentblock")` |
...
| `content`       | `ContentBlock` | 否  | 进展内容（富文本），见 `lark_get_skill(domain="okr", section="contentblock")` |
...
| 字段              | 类型             | 必填 | 说明                                                      |
|-----------------|----------------|----|---------------------------------------------------------|
| `content`       | `ContentBlock` | 是  | 进展内容（富文本），见 `lark_get_skill(domain="okr", section="contentblock")` |
...
> **SHORTCUT：**
> - `lark_okr_progress_get` (参见 `lark_get_skill(domain="okr", section="progress-get")`) 获取单条进展记录
...
- `lark_get_skill(domain="okr", section="contentblock")` — content/notes 字段的富文本结构定义
- `lark_get_skill(domain="okr", section="cycle-list")` — 列出用户 OKR 周期
- `lark_get_skill(domain="okr", section="cycle-detail")` — 获取周期下的目标与关键结果
- `lark_get_skill(domain="okr", section="progress-get")` — 获取进展记录
- `lark_get_skill(domain="okr", section="progress-create")` — 创建进展记录
- `lark_get_skill(domain="okr", section="progress-update")` — 更新进展记录
- `lark_get_skill(domain="okr", section="progress-delete")` — 删除进展记录
