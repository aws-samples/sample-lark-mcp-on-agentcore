...
- 设置会议 owner：`lark_calendar_create` 不支持，需用完整 API 命令在 `vchat.meeting_settings.owner_id` 中设置，且必须同时设置 `vchat.vc_type` 为 `vc`（代表该日程为 VC 视频会议）。⚠️ 该操作仅当以应用（bot）身份在应用日历上操作时生效，需要 bot 身份，MCP server 使用 user 身份，无法执行；owner 必须为用户身份（`ou_` open_id），不能为非用户或外部租户用户。
