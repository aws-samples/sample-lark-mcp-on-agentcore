...
进入此分支前，调度器（lark_get_skill(domain="calendar", section="schedule-meeting")）已完成：
- 任务类型判定（新建 / 编辑）
- 编辑流：目标 event_id 已定位
- 新建流：默认值已补全
- 时间已判定为**明确**
...
详见 lark_get_skill(domain="calendar", section="room-find")。
...
- **有冲突**：必须先说明冲突情况，询问用户：
  ...
  - **换时间** → 转入 lark_get_skill(domain="calendar", section="schedule-fuzzy-time")

## 落地

根据任务类型：
- 新建 → lark_get_skill(domain="calendar", section="create")
- 编辑 → lark_get_skill(domain="calendar", section="update")

落地规则详见 lark_get_skill(domain="calendar", section="schedule-meeting")（§ 落地日程变更）。
