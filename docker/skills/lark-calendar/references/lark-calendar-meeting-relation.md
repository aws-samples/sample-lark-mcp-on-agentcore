...
```
# 1. 当前时间的日程
lark_calendar_agenda(start="<now>", end="<now>")
...
#    （lark_vc_meeting_list_active / lark_vc_detail 结果的 id 字段即 meeting_id，无 event_id）
...
```
# 二选一：无关键词 → lark_calendar_agenda；有关键词 → lark_calendar_search_event
lark_calendar_agenda(start="<future_start>", end="<future_end>")
lark_calendar_search_event(query="<keyword>", start="<future_start>", end="<future_end>")
```

禁用：`lark_vc_search` 对未来返回空，容易被误判「没有会议」——不要用它查未来时间段。
...
lark_vc_search(start="<past_start>", end="<past_end>")

# 2. 过去的日程
lark_calendar_agenda(start="<past_start>", end="<past_end>")
...
| 用户输入 | 动作 |
|----------|------|
| 只给了「会议标题」 | 不确定是日程标题还是即时会议标题，**同时**查 `lark_calendar_search_event(query="<标题>")` 与 lark_get_skill(domain="meeting") 的 `lark_vc_search(query="<标题>")`，交叉后按上述意图分流 |
| 直接给了 `meeting_id` | 直接进入 lark_get_skill(domain="meeting")，跳过日程 |
| 相对锚点（「今天下午 3 点那个会」） | 先 `lark_calendar_agenda` 定位日程，再按意图 1 或 3 判断 |
| 过去锚点（「昨天开的会」） | **禁止只查 `lark_calendar_agenda`**——必须同时查 `lark_vc_search`，否则漏掉即时会议 |
| 未来锚点（「明天下午的会」） | 只查日程，不查 `lark_vc_search`（未来永远返回空） |
