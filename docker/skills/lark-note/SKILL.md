...
| `lark_docs_fetch` 返回 `<vc-transcribe-tab vc-node-id="...">` | 取 `vc-node-id` 作为 `NOTE_ID`，先 `lark_note_detail(note_id="NOTE_ID")` |
| 只持有 `meeting_id` | 先 `lark_vc_detail(meeting_ids="<id>")` 拿 `note_id`，再 `lark_note_detail(note_id="NOTE_ID")` |
| 只持有 `minute_token`（妙记 URL） | 先 `lark_minutes_detail(minute_tokens="<token>")` 顶层取 `note_id`，再 `lark_note_detail(note_id="NOTE_ID")`（不要把 `minute_token` 当 `note_id`） |
| 只持有日程 `event_id` | 先 `lark_calendar_meeting(event_ids="<id>")` 拿 `meeting_id`，再按上一行继续 |
| 已知 `note_id`，读纪要正文 | `lark_note_detail` → `lark_docs_fetch(doc="<note_doc_token>")` |
...
- 通过 `meeting_id` 定位纪要（`note_id`）→ `lark_get_skill(domain="vc")`（`lark_vc_detail`）。
- 通过 `minute_token` 定位纪要（`note_id`）→ `lark_get_skill(domain="minutes")`（`lark_minutes_detail` 顶层返回 `note_id`）。
- 通过日程 `event_id` 定位会议(`meeting_id`) / 用户绑定纪要(`meeting_note`) → `lark_get_skill(domain="calendar")`（`lark_calendar_meeting`）。
...
- **note_id**：纪要的唯一标识符，可通过 `lark_vc_detail(meeting_ids="...")` 获取。
...
1. 当用户已有 `note_id`，需要获取对应的 `note_doc_token`、`verbatim_doc_token` 或 `shared_doc_tokens` 时，使用 `lark_note_detail`。
2. `note_id` 通常来自 `lark_vc_detail` 的返回结果。
3. 获取到文档 Token 后，可使用 `lark_docs_fetch` 读取文档内容，或通过 `lark_invoke` 调用 `lark_drive_metas_batch_query` 获取文档元信息。
