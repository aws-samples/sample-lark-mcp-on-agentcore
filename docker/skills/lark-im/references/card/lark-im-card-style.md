...
选择组件组合和视觉样式的决策指南。字段写法见 `lark_get_skill(domain="im", section="card/card-2.0-schema")`。
...
- `token` 必须从 `lark_get_skill(domain="im", section="card/resource/icons")` 的精确枚举中选择；禁止根据名称规律自行拼接 token。没有合适的 token 时省略 icon。
