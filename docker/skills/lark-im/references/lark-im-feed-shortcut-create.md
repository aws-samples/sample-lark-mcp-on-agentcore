...
This tool maps to: `lark_im_feed_shortcut_create`. Underlying API: `POST /open-apis/im/v2/feed_shortcuts`.
...
- Currently only supports **user identity**; bot identity is not allowed by the server.
- If you only know a group name, resolve its `oc_xxx` first with `lark_im_chat_search` or `lark_im_chat_list`.

## Commands

```
# Add a single chat as a feed shortcut (defaults to head/top insertion)
lark_im_feed_shortcut_create(chat_id="oc_xxx")

# Add multiple chats; comma-separated
lark_im_feed_shortcut_create(chat_id="oc_a,oc_b,oc_c")
...
|------|------|------|
| `chat_id` | required | open_chat_id to add as a feed shortcut; comma-separated for multiple; **max 10 per call** |
...
The response is a batch ledger. A full success exits with `ok:true`. Any non-empty `failed_shortcuts` is a partial failure: stdout carries `ok:false`, and the full ledger remains machine-readable:
...
The tool adds a `reason_label` field next to each numeric `reason`:
...
- Scripts should check the top-level `ok` and ledger counts. Partial failures intentionally keep machine-readable success and failure details on stdout.
- To inspect the current shortcut list, use `lark_im_feed_shortcut_list`. To remove a shortcut, use `lark_im_feed_shortcut_remove`.
