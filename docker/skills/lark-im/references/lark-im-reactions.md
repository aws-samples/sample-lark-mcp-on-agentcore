...
> **Heads-up — don't reach for `batch_query` by default.** The four message-pulling tools (`lark_im_messages_mget`, `lark_im_chat_messages_list`, `lark_im_messages_search`, `lark_im_threads_messages_list`) already call `im.reactions.batch_query` automatically and attach the result as a `reactions` block on each message (replies inside `thread_replies` included). Use those tools for any "read reactions of messages I'm already pulling" task. Reach for the raw `batch_query` API only when you have a standalone `message_id` outside that pull flow. See `lark_get_skill(domain="im", section="message-enrichment")` for the contract.
...
> **Important:** These raw API commands accept structured input through `params` and `data` arguments. They do not expose typed parameters such as `message_id` or `reaction_type` directly.
...
```
lark_discover(query="im.reactions")
lark_discover(query="im.reactions.create")
lark_discover(query="im.reactions.list")
lark_discover(query="im.reactions.delete")
```

If your local build has already exposed the batch API in schema, also check:

```
lark_discover(query="im.reactions.batch_query")
```
...
```
lark_invoke(tool_name="lark_im_reactions_create", args={
...
```
lark_invoke(tool_name="lark_im_reactions_list", args={params: {"message_id": "om_xxx"}})
lark_invoke(tool_name="lark_im_reactions_list", args={params: {"message_id": "om_xxx", "reaction_type": "SMILE"}})
lark_invoke(tool_name="lark_im_reactions_list", args={params: {"message_id": "om_xxx", "page_size": 50}})
lark_invoke(tool_name="lark_im_reactions_list", args={params: {"message_id": "om_xxx", "page_token": "<PAGE_TOKEN>"}})
lark_invoke(tool_name="lark_im_reactions_list", args={params: {"message_id": "om_xxx", "user_id_type": "open_id"}})
```
...
```
lark_invoke(tool_name="lark_im_reactions_delete", args={
...
```
lark_invoke(tool_name="lark_im_reactions_batch_query", args={
