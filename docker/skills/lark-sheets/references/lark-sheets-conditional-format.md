...
飞书表格的"颜色标记"语义 = 条件格式规则 ≠ 静态背景色。如果用 `lark_sheets_cells_set` 写静态，源数据变化时颜色不会跟着变（典型反例：用户要求"过期单元格标红"时，模型用静态填充——日期变化后单元格颜色不再准确反映过期状态）。

**判断标准**：交付后 `lark_sheets_cond_format_list` 必须能返回该规则；否则视为违规。

**大数据量首选**：当数据量 > 1000 行时，条件格式是首选——它由飞书自身渲染，比"本地脚本逐行计算 + `lark_sheets_cells_set` 写静态背景色"更高效、更稳（颜色还能随源数据自动联动）。
...
⚠️ **创建条件格式前必须读数据行确认列对应**：仅读首行表头（`lark_sheets_csv_get range="A1:Z1"`）不够——如果表头语义含糊（比如"时间"、"日期"这种多列同义词），formula 里引用的列字母可能张冠李戴。必须再读 3-5 行**数据样本**（如 `range="A2:Z6"`）确认：①列名对应的实际值；②字段含义匹配用户描述；③数据类型是日期/数字/文本。特别是比较类条件格式（`=$A2>$B2` 这种），列字母选错整条规则就废了。
...
| 参数 | Type | 必填 | 说明 |
| --- | --- | --- | --- |
| `properties` | object（复合 JSON） | required | 规则配置 JSON，含 `style`（命中样式，必填）和 `attrs?`（规则参数列表，因 `rule_type` 不同结构而异）/ `has_ref?`。`rule_type` 和 `ranges` 已拎为独立参数 |
| `rule_type` | string | required | 条件格式规则类型；优先级高于 `properties` 中同名字段（可选值：`duplicateValues` / `uniqueValues` / `cellIs` / `containsText` / `timePeriod` / `containsBlanks` / `notContainsBlanks` / `dataBar` / `colorScale` / `rank` / `aboveAverage` / `expression` / `iconSet`） |
| `ranges` | array（简单 JSON） | required | 应用条件格式的 A1 范围 JSON 数组（如 `["A1:A100","C2:C50"]`）；优先级高于 `properties` 中同名字段 |
...
| 参数 | Type | 必填 | 说明 |
| --- | --- | --- | --- |
| `rule_id` | string | required | 目标规则 id |
| `properties` | object（复合 JSON） | required | 规则配置 JSON，结构同 `lark_sheets_cond_format_create` 的 `properties`；update 是整组覆盖式 |
| `rule_type` | string | required | 条件格式规则类型；优先级高于 `properties` 中同名字段（可选值：`duplicateValues` / `uniqueValues` / `cellIs` / `containsText` / `timePeriod` / `containsBlanks` / `notContainsBlanks` / `dataBar` / `colorScale` / `rank` / `aboveAverage` / `expression` / `iconSet`） |
| `ranges` | array（简单 JSON） | required | 应用条件格式的 A1 范围 JSON 数组（如 `["A1:A100","C2:C50"]`）；优先级高于 `properties` 中同名字段 |
...
> 复合 JSON 参数字段速查（只列顶层 + 一层嵌套）。深层结构看下方 `## Examples`，或用 `lark_discover(query="sheets.cond_format_create")` 读完整 JSON Schema。

### `lark_sheets_cond_format_create` `properties` / `lark_sheets_cond_format_update` `properties`

_创建/更新的条件格式属性_

**顶层字段**：
- `rule_type` (enum) — 条件格式规则类型 [duplicateValues / uniqueValues / cellIs / containsText / timePeriod / containsBlanks / notContainsBlanks / dataBar / colorScale / rank / aboveAverage / expression / iconSet] — ⚠️ 已拎为独立参数 `rule_type`，请勿在此 JSON 内重复填写（同名以独立参数为准）
- `ranges` (array<string>) — 应用条件格式的 A1 范围列表 — ⚠️ 已拎为独立参数 `ranges`，请勿在此 JSON 内重复填写（同名以独立参数为准）
...
`rule_type` / `ranges` 是独立参数（不要再放 `properties`）；`style` / `attrs` 等结构走 `properties`：

```
# 重复值高亮
lark_sheets_cond_format_create(url="...", sheet_id="<SID>",
  rule_type="duplicateValues", ranges=["A1:A100"],
...
整组覆盖式：先 `lark_sheets_cond_format_list(rule_id="<id>")` 拿当前完整配置，改后整组传回。
