...
> **边界**：本文只讲"样式长什么样、怎么决策"；**怎么调用工具写入样式**（`cell_styles` / `border_styles` 字段、合并、resize 等参数）见 `lark_get_skill(domain="sheets", section="write-cells")` / `lark_get_skill(domain="sheets", section="range-operations")` / `lark_get_skill(domain="sheets", section="batch-update")`。**条件格式**（高亮 / 标红 / 数据条 / 色阶）见 `lark_get_skill(domain="sheets", section="conditional-format")`。本文不含 shortcut，通用编辑准则见主 SKILL.md「飞书表格编辑准则」。
...
- **美化范围必须覆盖所有用户语义目标**：用户说"给表格加边框 / 美化整个表"时，范围 = 实际数据区域**含所有数据行**（含汇总行、总计行、表尾备注行），不能停在"看起来主体内容结束"的地方。落地前先用 `current_region` + 末尾 5~10 行核对真实末行（同 `lark_get_skill(domain="sheets", section="read-data")` 的「确定数据范围的正确流程」），再设置美化范围。范围漏掉用户提到的目标行 / 列**直接判失败**。
...
5. **列宽 + 行高 + 自动换行**：详细规则见 `lark_get_skill(domain="sheets", section="range-operations")` 的「写入后列宽自适应」章节（按最长字符数扩列宽 / 长文本设置 `cell_styles.word_wrap="auto-wrap"` + 调高行高 / 长数字设置 `number_format` 防科学计数法）
...
- **列宽 / 行高调整**（飞书 `lark_sheets_cols_resize` / `lark_sheets_rows_resize` 直接给像素值：统一尺寸用 `range` + `width`/`height`（像素），多列 / 多行不同尺寸用 `widths`/`heights` map 一次调用完成，如 `widths={"A":100,"C:E":120}`）：
...
- **图表放置防重叠**：新增图表前须计算放置区域，避免与已有图表重叠。具体步骤：
  1. 调用 `lark_sheets_chart_list` 获取当前工作表所有已有图表的 `position`（锚点单元格：`col` 是列字母如 "A"/"B"、`row` 是 1-based 行号；以 `lark_sheets_chart_list` 实际返回字段为准）、`offset`（锚点内偏移：`row_offset`、`col_offset`，单位像素）以及 `size`（`width`、`height`，单位像素）。
  2. 获取工作表的行高和列宽信息（像素）。
...
> 具体工具调用参数格式，请读取对应工具 skill（`lark_get_skill(domain="sheets", section="write-cells")`、`lark_get_skill(domain="sheets", section="conditional-format")`、`lark_get_skill(domain="sheets", section="range-operations")` 等）。
...
**决策流程：**
1. 先用 `lark_sheets_cells_get` 读取目标位置上方的数据区域，确认数据边界和已有样式（背景色、字体大小等）
2. 如果需要新增空行，先用 `lark_sheets_dim_{insert|delete|hide|unhide|freeze|group|ungroup}` 插入行
3. 用 `lark_sheets_cells_set` 写入汇总公式 + 特殊样式（背景色区分 + 加粗 + 边框）
4. 如果汇总行标题需要跨列显示，追加 `lark_sheets_cells_{merge|unmerge}` 合并标题区域
...
1. 新建 sheet，或用 `lark_sheets_cells_get` 或 `lark_sheets_workbook_info` 确认已有表格的占用范围，找到空闲区域
2. 用 `lark_sheets_cells_get` 采样已有表格的表头样式（背景色、字体大小、字重、对齐方式）和数据区域样式
3. 新表头复用已有表头的配色和字体参数（保持风格统一），但内容和列宽可独立
4. 新数据区域复用已有数据区域的对齐规则、边框风格、数字格式
5. 用 `lark_sheets_cells_set` 一次性写入新表头 + 数据
...
> 具体继承哪些字段、怎么采样与写入（`lark_sheets_cells_get` 读源行 `cell_styles` + `border_styles`、`lark_sheets_sheet_info(include="row_heights,merges")` 读行高合并、带齐 6 类样式写入）见 `lark_get_skill(domain="sheets", section="write-cells")` 的「新增列 / 新增行的样式继承」章节——`border_styles` 四边最易遗漏，以那里为准。
...
```
Step 1 — 格式铺开：`lark_sheets_batch_update` + `lark_sheets_range_copy`（或 `lark_sheets_range_fill`）
  └── 将模板行/区域的 **全部格式**（样式、边框、数字格式、数据验证等）复制到目标区域
  └── 推荐用 `lark_sheets_range_copy(paste_type="formats")`（仅复制格式，目标值/公式保留），即"格式刷"
  └── 若需连带公式平移填充（如公式列结构一致），改用 `lark_sheets_range_fill(series_type="copy")` 或 `lark_sheets_range_copy(paste_type="all")`

Step 2 — 内容覆写：`lark_sheets_batch_update` + `lark_sheets_cells_set`（仅传 value/formula，不传任何样式）
  └── 将每行的实际数据写入，cell_styles 全部省略，因为格式已在 Step 1 中就位

Step 3 — 微调收尾：`lark_sheets_rows_resize(heights=…)` / `lark_sheets_cols_resize(widths=…)`（行高列宽 map 一次调用完成）、`lark_sheets_batch_update` + `lark_sheets_cells_{merge|unmerge}` 等
  └── 调整行高列宽、处理合并单元格、扩展条件格式范围等边缘情况
```

**关键注意事项：**
- Step 1 用 `lark_sheets_range_copy(paste_type="formats")` 时只铺格式、不动值/公式，Step 2 再用 `lark_sheets_cells_set` 写值即可（`lark_sheets_cells_set` 默认覆盖，无需额外 flag）；若 Step 1 用 `paste_type="all"` 连带复制了值/公式，Step 2 写入同样会覆盖（默认行为）
- `lark_sheets_range_fill(series_type="auto")`（或 `linear`/`date`）会自动递增数字序列（1→2→3）和日期序列，`lark_sheets_range_fill(series_type="copy")` 则原样复制值但公式引用会自动平移
- 如果模板区域存在合并单元格，copy/fill 不会复制合并状态，必须在 Step 3 中用 `lark_sheets_cells_{merge|unmerge}` 补全
- 如果模板区域有条件格式，需要在 Step 3 中通过 `lark_sheets_cond_format_update` 扩展 ranges

**场景：纯"格式刷"（用户说"把 A 列样式应用到 B 列"、"格式复制过去"、"只刷格式不改数据"）**

单步即可，无需三步分层：调用 `lark_sheets_range_copy(paste_type="formats")`，`source_range` 为样式来源、`target_range` 为目标起点。参数细节见 `lark_get_skill(domain="sheets", section="range-operations")`。
...
```
1. 探查阶段
   ├── `lark_sheets_workbook_info` → 获取子表列表、行列数、冻结位置
   ├── `lark_sheets_sheet_info(include="merges")` → 获取合并区域
   ├── `lark_sheets_cells_get`（前几行 + 末尾几行，`include="style"`）→ 采样表头/数据区/汇总行样式
   └── 分析结果 → 建立区域地图（表头行号、数据起止行号、汇总行号、合并区域列表）

2. 规划阶段
   ├── 判断表头行：通常第 1 行或前 2 行，特征为加粗/背景色/合并/居中
   ├── 判断汇总行：通常最后 1~2 行，特征为加粗/SUM/AVERAGE 公式/更深背景色
   ├── 判断合并区域：从 `lark_sheets_cells_get` 返回中识别（多个单元格同值且样式相同通常暗示合并）
   └── 制定美化方案：按区域分别设置样式
...
> 合并单元格完整的安全操作规则（含数据保护、样式占位等 5 条）见 `lark_get_skill(domain="sheets", section="range-operations")` 的 `lark_sheets_cells_{merge|unmerge}` 章节。
