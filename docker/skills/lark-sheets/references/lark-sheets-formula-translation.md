...
> **边界**：本文只讲"公式怎么写对"；公式**怎么写入表格**（`lark_sheets_cells_set` / 模板单元格 + `copy_to_range` / 容错回读）见 `lark_get_skill(domain="sheets", section="write-cells")`。公式写入完成后可用 `lark_get_skill(domain="sheets", section="formula-verify")` 做诊断；不要把"翻译对了"误当成"结果一定正确"。本文不含具体工具，通用编辑准则见主 SKILL.md「飞书表格编辑准则」。

**核心原则一：飞书不像 Excel 365 那样默认 spill（溢出展开）。** 某个参数要求单值、实际传入的却是区域时，飞书默认取"投影"（按公式所在行/列取对应的那一个值）；只有当求值处于**数组公式上下文内部**——最外层套了 `ARRAYFORMULA`，或公式里已有原生数组函数（`FILTER` / `XLOOKUP` / `SORT` 等，见下方清单）——才逐项展开。`ARRAYFORMULA` 与"逐行标量公式 + `copy_to_range` 填充"导出后都保真，按需要选：前者一条公式覆盖整片、写起来短；后者每格是独立公式，导出后在 Excel 里能单格编辑。
...
- **公式字符串用飞书范围语法**：写 `H:H`、`A2:B5`，**禁止** `H2:H` / `2:2`。要在公式里引用整行，用显式范围（如 `$A2:$Z2`）替代禁用的 `2:2`。这与工具参数（如 `range` / `copy_to_range`）的 A1 表示法写法不同：参数侧合法的 `D3:D`、`1:1`、`3:6` 在公式串里反而非法。**公式串 ≠ 工具参数**，两套规则别互相照搬，混用会导致调用失败或公式报错。
...
1. 按本文完成公式改写后，用 `lark_get_skill(domain="sheets", section="write-cells")` / `lark_get_skill(domain="sheets", section="batch-update")` 把公式真实写入表格。
2. 公式一旦落表，可进入 `lark_get_skill(domain="sheets", section="formula-verify")` 做诊断。
3. `lark_sheets_formula_verify` 的 `errors_found` / `partial` 是风险信号；关键输出区优先修复，非关键区可在交付说明记录。
...
2. 最终结果是**一维或二维数组**：
   ...
   - 公式中**不包含**任何原生数组函数，只是在对区域做标量计算 → 用 `ARRAYFORMULA` 包住整个表达式，或写成**单行标量公式再向下 / 向右填充**（`copy_to_range`）
...
- **逐行标量式 + 填充**：首行写 `=A2*B2` / `=IF(A2>0,B2,"")`，再用 `copy_to_range` 铺到整列，引用随行递增。每格是独立公式，导出后在 Excel 里能单格编辑
...
飞书公式没有跨工作簿引用的通用写法（Excel 的外部链接迁过来也不成立）。需要另一份电子表格的数据时，先把那份数据读出来（`lark_sheets_csv_get` 等）落到本表的一张子表，再在本表内用普通引用计算——既避开跨表引用限制，也保证导出后公式仍可用。
...
辅助列首行（如 Z16）：=单行计算逻辑          # 例：=SUBTOTAL(103,INDIRECT("E"&ROW(E16)))
                        用 copy_to_range 铺满 Z16:Z387（引用随行递增）
...
- `PIVOT` — 用 `lark_sheets_pivot_{create|update|delete}` 透视表对象替代
...
- 基础逐项计算
  ...
  - 飞书: `=ARRAYFORMULA(A2:A100*B2:B100)`；或首行 `=A2*B2` + `copy_to_range` 向下填充
- 条件判断
