...
`select` 字段用 `multiple` 区分单选和多选：`multiple=false` 时传选项名字符串，`multiple=true` 时传选项名数组。只支持写入字段中已有的选项；构造 CellValue 前先用 `lark_base_field_list()` 或 `lark_base_field_search_options()` 确认目标选项存在。
