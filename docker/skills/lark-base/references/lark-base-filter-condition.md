...
Filter 是一组「字段/操作符/值」条件的组合，用 `logic`（`and` / `or`）把多条 `conditions` 连接起来，用于描述「满足什么条件」。视图筛选 `filter`、记录读取/搜索的 `filter_json`、表单题目显隐条件 `visible_rule` 复用同一套 tuple 结构，本文件是其公共协议（SSOT）。

## 0. 适用范围

本协议只适用于以下场景：

- `lark_base_view_set_filter()` / `lark_base_view_get_filter()` 的视图筛选配置。
- `lark_base_record_list(filter_json=...)` / `lark_base_record_search(filter_json=...)` 的结构化记录筛选。
- `lark_base_form_questions_create()` / `lark_base_form_questions_update()` 中的 `visible_rule` 显隐条件。

本协议**不适用于 `lark_base_data_query()`**。`lark_base_data_query()` 支持过滤，但使用的是 LiteQuery DSL 的 `filters` 对象结构：`{"type":1,"conjunction":"and","conditions":[{"field_name":"状态","operator":"is","value":["有效"]}]}`，不是这里的 tuple 条件 `["状态","==","有效"]`。构造 `lark_base_data_query(dsl=...)` 时请阅读 `lark_get_skill(domain="base", section="data-query")` 的 FilterGroup / Condition 章节。
...
`lark_base_record_list()` / `lark_base_record_search()` 的 `filter_json` 也支持使用与视图相同的 tuple condition。以下示例用注释说明各条件的含义；实际传参时删除注释并使用标准 JSON：
...
> **人员筛选：不要猜 ID。** 不知道 `open_id` 时，先用 `lark_contact_search_user(query="<姓名/邮箱/手机号>")` 查 id。
...
> **群组筛选：不要猜 ID。** 不知道 `chat_id` 时，先用 `lark_im_chat_search(query="<群名关键词>")` 搜群；取结果里的 `oc_xxx`。
