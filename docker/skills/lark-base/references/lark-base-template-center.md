...
- `lark_base_template_list()`：列出某个分类下的模板（不传分类则返回"推荐"类目）。
- `lark_base_template_search()`：按关键词搜索模板。
...
- 用户给了 Base/Wiki 链接或 token → 走 `lark_base_url_resolve()`。
- 用户说"我之前那张表 / 我的模板 / 最近访问的" → 走 `lark_base_title_resolve()` 或用 `lark_drive_search()` 列候选。
- 用户要从零定义字段 schema，而不是套现成模板 → 走 `lark_base_base_create(table_name="<table>", fields="<field-json-array>")`。

模板中心是独立的公开数据集，**不能**用 `lark_drive_search()` 找到，`lark_drive_search()` 只搜用户自己可访问的云空间对象。

## 推荐调用

```
# 列出所有模板分类
lark_base_template_categories()

# 列出某个分类下的模板（不传 category_key 则返回"推荐"类目）
lark_base_template_list(category_key="template_center_tab_ai", limit=10)

# 按关键词搜索模板
lark_base_template_search(keyword="项目管理", limit=10)

# 翻页：把上一页返回的 offset 原样传给 offset
lark_base_template_search(keyword="AI", limit=10, offset="<上一页返回的 offset>")

# 选定模板后，用模板 token 复制成用户自己的新 Base
lark_base_base_copy(base_token="<模板 token>", name="<新 Base 名>")
```
...
1. `lark_base_template_categories()` 列出全部分类，拿到 `categories[].key` 和 `name`。
2. AI 把用户意图匹配到最贴近的一个分类 `name`，取它的 `key`。
3. `lark_base_template_list(category_key="<key>")` 列出该类目下的模板。
4. 读每个模板的 `name` / `introduction` / `scenarios`，挑出最符合用户场景的那个，拿它的 `token`。
5. 用 `lark_base_base_copy(base_token="<token>")` 基于模板复制出新 Base（见下文"基于模板创建"）。
...
1. `lark_base_template_search(keyword="<词>")` 搜模板。
2. 同样读 `name` / `introduction` / `scenarios` 选模板，拿 `token`。
3. `lark_base_base_copy()` 复制。
...
| 用户意图 | 走哪条 |
|---|---|
| 只有大类方向（"市场营销类的""办公用的"） | 路径 A，先 `lark_base_template_categories()` 收敛 |
| 有具体、可检索的业务词（"报销""OKR""直播"） | 路径 B，直接 `lark_base_template_search()` |
| 大方向下没挑到合适的 | A 之后再用 B 换关键词补搜 |

## 翻页

`lark_base_template_list()` 和 `lark_base_template_search()` 都是游标翻页：
...
`lark_base_template_categories()` 返回 `categories[]`，每个元素：

| 字段 | 类型 | 含义 |
|---|---|---|
| `key` | string | 分类唯一标识，形如 `template_center_tab_ai`（`template_center_tab_` 前缀 + 类目名）。传给 `lark_base_template_list()` 的 `category_key` 用的就是它 |
| `name` | string | 分类展示名，如 `AI 应用` / `办公通用`。AI 匹配用户意图时看这个 |

### Template（模板对象）

`lark_base_template_list()` / `lark_base_template_search()` 返回 `templates[]`，每个元素：

| 字段 | 类型 | 含义 |
|---|---|---|
| `token` | string | 模板的 Base token，是模板的唯一标识。基于模板创建时作为 `lark_base_base_copy()` 的 `base_token` 入参 |
...
- 三个模板中心工具都是只读，所需权限 `base:template:read`（认证由 MCP server 自动处理）。
- 模板中心是公开数据集，不能用 `lark_drive_search()` 找到；用户要"我的/最近访问/已有 Base"不要走这里。
- 分类先于列表：`lark_base_template_list()` 的 `category_key` 必须来自 `lark_base_template_categories()` 的返回，不要凭空猜类目 key。
- `lark_base_template_search()` 不支持空关键词，会被拒绝；用户只有大方向、无具体检索词时改走分类浏览。
