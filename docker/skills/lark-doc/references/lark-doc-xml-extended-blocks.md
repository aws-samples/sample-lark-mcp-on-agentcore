...
本文件用于补充说明 block XML 扩展能力。常用标签和通用规则见 `lark_get_skill(domain="doc", section="xml")`；后续新增其他 block 说明时可继续追加到本文件。
...
1. 写入 HTML 内容块时，HTML 一律**内联**在 `<html5-block>` 标签里（⚠️ `path="@./widget.html"` 这类本地文件引用在 MCP server 上不可用，容器里没有 agent 可写的文件系统）；已有 `data-ref` 时配合 `reference_map` 参数传入结构化 JSON。读取时 `<html5-block data-ref="html5_1"></html5-block>` 只是占位，必须从 `document.reference_map["html5-block"]["html5_1"].data` 读取 HTML；若 entry 是 `path`，读取对应 `@doc-fetch-resources/...html` 文件。
2. 格式如下：
...
只使用 `auto` 或 `viewport`：正文需要在文档中完整展开时使用 `auto`；内容需要在 HTML Block 内滚动或单屏呈现时使用 `viewport`。HTML 会被原样写入 `reference_map`，不会校验该字段，因此创建或更新前必须在 `<head>` 中显式声明。
...
- 页面加载后的内容追加或展开不会自动刷新高度，不要臆造相关参数。
...
OKR block 可用 XML 格式完整表达。创建前先参考 `lark_get_skill(domain="okr")` 确认可用周期；创建时只写 root-only `<okr cycle-id="..."/>` 挂载已有 OKR，不构造 Objective/KR/Progress 子树。
