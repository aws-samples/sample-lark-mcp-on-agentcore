...
解读已定义指标、趋势、分布、漏斗、监控、估计或实验观察值。`有数据`、`有数字`、`分析一下`单独不决定路由；研究问题、抽样和可推广性为主走 `lark_get_skill(domain="doc", section="genres/research-report")`，比较商业选项走 `lark_get_skill(domain="doc", section="genres/business-analysis")`，组织状态、偏差和下一步走 Workplace 周期报告。
