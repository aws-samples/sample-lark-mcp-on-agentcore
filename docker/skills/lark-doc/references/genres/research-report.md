...
以明确研究问题、研究设计或材料、发现和限制为主要交付。`调研`、`研究过`、`访谈`、`问卷`单独只用于召回；只解读既定指标走 `lark_get_skill(domain="doc", section="genres/data-report")`，比较特定战略或资源选项走 `lark_get_skill(domain="doc", section="genres/business-analysis")`，方法并非判断重点的问题框架综合走 `lark_get_skill(domain="doc", section="genres/white-paper")`。
