...
- 如果需要服务端生成的预览效果，例如 doc/docx 的 PDF 版式预览，先用 `list_only=true` 查看候选项，再按候选项选择 `type="pdf"` / `"text"` / `"image"` 等
- 想下载时，必须显式传 `type` 和 `output`
- 如果 `list_only=true` 没有可用预览候选项，或错误提示明确建议使用 `type="source_file"`，可以改用 `type="source_file"` + `output="<path>"` 查看文件内容；资源不存在、token 无效等终态错误需要先修正输入
