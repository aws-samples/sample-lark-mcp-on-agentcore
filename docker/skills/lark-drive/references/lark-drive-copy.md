...
lark_drive_copy(url="https://example.larksuite.com/docx/<DOCX_TOKEN>", name="副本名称", folder_token="<TARGET_FOLDER_TOKEN>")

# Wiki URL（自动解包底层资源后复制到 Drive）
lark_drive_copy(url="https://example.larksuite.com/wiki/<WIKI_TOKEN>", name="副本名称", folder_token="<TARGET_FOLDER_TOKEN>")

# Wiki token
lark_drive_copy(token="<WIKI_TOKEN>", type="wiki", name="副本名称", folder_token="my_space")
```
...
- `base` 与 `bitable` 是同一概念，会把 `base` 归一化为 `bitable` 后发给服务端
- 目标文件夹必须是云空间（云盘/云存储）文件夹 token，不能传 wiki 节点 token

## Wiki 场景

`lark_drive_copy` 接受 wiki URL，也接受 `token="<WIKI_TOKEN>", type="wiki"`。目标仅支持云盘（Drive）文件夹或 `my_space` 根目录；要把副本留在知识库中，使用 `lark_wiki_node_copy`。

## 行为说明

- ⚠️ `data.permission_grant` 字段只在 bot 身份复制时出现（复制成功后自动给当前用户授予新副本 `full_access`）。MCP server 始终使用 user identity，因此该字段不会出现在返回里。
...
- `lark_get_skill(domain="wiki")` -- 知识库节点复制（`lark_wiki_node_copy`）
