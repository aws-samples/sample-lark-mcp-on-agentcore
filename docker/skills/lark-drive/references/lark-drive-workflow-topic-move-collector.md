...
本文档实现已注册的主题资料收集 workflow。执行前必须先调用 `lark_get_skill(domain="drive", section="workflow")`，并遵循共享执行协议、Artifact Contract、Workflow Loading、认证和写入确认规则（认证由 MCP server 自动处理）。
...
执行本 workflow 前，身份、认证、权限和写操作确认规则由 MCP server 自动处理。

按阶段渐进加载其他 skill / 引用文档：

- 目标是 Wiki 或个人文档库：`lark_get_skill(domain="wiki")`
- 需要读取文档内容：`lark_get_skill(domain="doc")` 和 `lark_get_skill(domain="doc", section="fetch")`
- 需要验证 Sheet 内容：`lark_get_skill(domain="sheets")`
- 需要 Drive 搜索：`lark_get_skill(domain="drive", section="search")`
- 需要资源解析：`lark_get_skill(domain="drive", section="inspect")`
...
不要求用户先限定文件夹或知识库范围。只有用户明确指定范围时，才使用 `folder_tokens`、`space_ids` 或其他显式限制。
...
| 文件 | 负责 | 不负责 |
|------|------|--------------|
| `workflow-topic-move-collector` | 触发规则、全局约束、状态机、渐进加载关系、命令族白名单 | 具体阶段规则、UI 模板、执行细节 |
| `workflow-topic-move-collector-setup` | `PARSE_INPUT`、`RESOLVE_TARGET`、`CONFIRM_CONTEXT`、`TargetLocation` | 搜索执行、相关性分类、写操作 |
| `workflow-topic-move-collector-recall` | `SEARCH_RECALL`、`RECALL_ENHANCE`、搜索 query 策略、去重、`CandidateItem` | 资源 token 解析、内容验证、写操作 |
| `workflow-topic-move-collector-resolve-verify` | `RESOURCE_RESOLVE`、`CONTENT_VERIFY`、权限矩阵、`ResourceItem` | 相关性分类、移动计划、写操作 |
| `workflow-topic-move-collector-review-plan` | `RELEVANCE_CLASSIFY`、`PLAN_MOVE`、`MovePlanItem`、展示分组 | 资源解析、内容验证、写操作执行、恢复 |
| `workflow-topic-move-collector-execute` | `CONFIRM_EXECUTION`、`EXECUTE`、`VERIFY`、`RESTORE`、`RollbackSnapshotItem`、执行日志 | 搜索、分类和计划 schema |
...
| 状态 | 必读文档 |
|-------|---------------|
| `PARSE_INPUT` / `RESOLVE_TARGET` / `CONFIRM_CONTEXT` | `lark_get_skill(domain="drive", section="workflow-topic-move-collector-setup")` |
| `SEARCH_RECALL` / `RECALL_ENHANCE` | `lark_get_skill(domain="drive", section="workflow-topic-move-collector-recall")` |
| `RESOURCE_RESOLVE` / `CONTENT_VERIFY` | `lark_get_skill(domain="drive", section="workflow-topic-move-collector-resolve-verify")` |
| `RELEVANCE_CLASSIFY` / `PLAN_MOVE` | `lark_get_skill(domain="drive", section="workflow-topic-move-collector-review-plan")` |
| `CONFIRM_EXECUTION` / `EXECUTE` / `VERIFY` / `RESTORE` | `lark_get_skill(domain="drive", section="workflow-topic-move-collector-execute")` |
...
- 输入与目标确认：`lark_get_skill(domain="drive", section="workflow-topic-move-collector-setup")`
- 召回：`lark_get_skill(domain="drive", section="workflow-topic-move-collector-recall")`
- 资源解析与内容验证：`lark_get_skill(domain="drive", section="workflow-topic-move-collector-resolve-verify")`
- 审核与计划：`lark_get_skill(domain="drive", section="workflow-topic-move-collector-review-plan")`
- 执行：`lark_get_skill(domain="drive", section="workflow-topic-move-collector-execute")`
- `lark_get_skill(domain="drive", section="search")`
- `lark_get_skill(domain="drive", section="inspect")`
- `lark_get_skill(domain="drive", section="move")`
- `lark_get_skill(domain="drive", section="create-folder")`
- `lark_get_skill(domain="drive", section="delete")`
- `lark_get_skill(domain="wiki", section="move")`
- `lark_get_skill(domain="wiki", section="move-to-drive")`
- `lark_get_skill(domain="wiki", section="node-create")`
- `lark_get_skill(domain="wiki", section="node-delete")`
