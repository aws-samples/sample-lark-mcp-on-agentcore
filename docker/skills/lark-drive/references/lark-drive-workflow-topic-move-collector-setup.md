...
2. 解析 Drive 目标时，遵循 `lark_get_skill(domain="drive", section="inspect")`、`lark_get_skill(domain="drive", section="create-folder")` 和 `lark_get_skill(domain="drive", section="search")`。
3. 解析 Wiki 目标时，遵循 `lark_get_skill(domain="wiki")`、`lark_get_skill(domain="wiki", section="node-get")` 和 `lark_get_skill(domain="wiki", section="node-create")`。
...
|-----------|---------------|-------------------|
| 已有 Drive 文件夹 URL 或 token | 有 URL 时用 `lark_drive_inspect` 解析；保留 `folder_token` | `drive_folder` |
| 已有 Wiki 节点 URL 或 token | 用 `lark_wiki_node_get` 或 `lark_drive_inspect` 解析；保留 `wiki_node_token` 和 `space_id` | `wiki_node` |
...
| 来源 -> 目标 | 默认规则 |
|------------------|---------|
| Drive 资源 -> Drive 文件夹 | 支持，使用 `lark_drive_move`。 |
| Drive 文档类资源 -> Wiki 节点 / 空间 | 资源类型支持时，使用 `lark_wiki_move`。 |
| Wiki 节点 -> Wiki 节点 / 空间 | 支持，使用 `lark_wiki_move`（`node_token` 模式）。 |
| Wiki 节点 -> Drive 文件夹 | `lark_wiki_move_to_drive`。 |
