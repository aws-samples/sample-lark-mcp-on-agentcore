...
本文实现已注册的权限治理 workflow。执行前必须先调用 `lark_get_skill(domain="drive", section="workflow")`，并遵循共享执行协议、Artifact Contract、Workflow Loading 和写入确认规则。（认证、全局参数和安全规则由 MCP server 自动处理。）
...
- 文件夹自身公开权限审计或修复。文件夹自身权限设置可以用 `lark_drive_permission_get_setting` 读取；写入是否支持必须以运行时 schema 和明确需求为准，不能猜测执行 `lark_drive_permission_public_patch` 的 `type=folder`。
- 当前身份无法枚举到的不可见文档的完整发现；只能处理已发现目标，或用户显式提供的 URL / token。
- 未按范围确认的批量写入。

协作者列表读取只覆盖当前目标的直接协作者/授权成员：可使用 `lark_drive_member_list`。

## Progressive Load Map

本表只规定每个 state 需要加载的额外上下文；命令可用范围以 `Command Map` 为准。需要拼装具体命令时，再按需调用 `lark_get_skill(domain="drive", section="workflow-permission-governance-commands")`。

| State | Required Reference |
|-------|--------------------|
| `PARSE_INTENT` | 本文件、`lark_get_skill(domain="drive", section="workflow")` |
| `TARGET_INSPECT` | `lark_get_skill(domain="drive", section="inspect")` |
| `DISCOVER_TARGETS` | 容器范围时调用 `lark_get_skill(domain="wiki", section="node-list")` 或 `lark_get_skill(domain="drive", section="files-list")` |
| `FACT_READ` | `lark_discover(query="drive.metas.batch_query")`；涉及权限设置读取时使用 `lark_drive_permission_get_setting`（详见 `lark_get_skill(domain="drive", section="permission-get-setting")`）；涉及活跃度、访问复核或生命周期判断时再调用 `lark_discover(query="drive.file.statistics.get")` 和 `lark_discover(query="drive.file.view_records.list")` |
| `RISK_ASSESS` | 本文件的 `Risk Classification` |
| `EXEC_CONFIRM` | 只为用户选择的动作调用 `lark_get_skill(domain="drive", section="apply-permission")`、`lark_get_skill(domain="drive", section="secure-label")`，或 `lark_discover(query="drive.permission.public.patch")` / `lark_discover(query="drive.permission.members.transfer_owner")`；需要确认模板时调用 `lark_get_skill(domain="drive", section="workflow-permission-governance-outputs")` |
...
| `TARGET_INSPECT` | `scope` | 解析单资源、明确列表、Wiki space / node、Drive folder；Drive folder 直接从 URL 路径或显式 `type="folder"` 解析，不调用 `lark_drive_inspect`；保留原始 URL、scope type、canonical token/type | 目标范围表，包含 scope、title/type/token status | 除非解析失败，否则为 `false` | `DISCOVER_TARGETS` or `FACT_READ` |
...
| `FACT_READ` | `read` | 对直接目标或 `discovered_targets` 执行 `drive metas batch_query`；对支持的文件、文件夹或云文档目标执行 `lark_drive_permission_get_setting` 读取自身权限设置；当 `intent=public_exposure_check` 且 `target_scope=single_resource` 时，可复用 `lark_drive_inspect` 返回的 title / URL / type，只补读目标公共访问和协作权限设置；在用户要求活跃度 / 访问复核 / 生命周期判断时读取访问统计和访问记录 | 权限事实摘要、coverage summary、activity facts 和 unsupported checks | 除非所有目标都被 auth 阻断，否则为 `false` | `RISK_ASSESS` |
...
|-------|--------------------------|---------|
| `TARGET_INSPECT` | `lark_drive_inspect` | 解析非 folder URL、type、canonical token、title 和 wiki unwrap data；Drive folder 不支持 `lark_drive_inspect`，必须从 URL 路径或显式 `type="folder"` 直接解析 |
| `DISCOVER_TARGETS` | `lark_wiki_node_list` | 递归发现 Wiki space / node 下当前身份可见的节点 |
| `DISCOVER_TARGETS` | `lark_invoke(tool_name="lark_drive_files_list")` | 递归发现 Drive folder 下当前身份可见的文件和子文件夹 |
| `FACT_READ` | `lark_invoke(tool_name="lark_drive_metas_batch_query")` | 读取 title、URL、owner 和 secure-label metadata |
| `FACT_READ` | `lark_invoke(tool_name="lark_drive_permission_public_get")` | 读取支持类型的目标公共访问和协作权限设置，包括链接分享、对外分享、协作者管理、复制内容、创建副本、打印、下载和评论 |
| `FACT_READ` | `lark_drive_member_list` | 读取用户显式要求的单目标直接协作者/授权成员列表；不代表完整继承链或历史权限审计 |
| `FACT_READ` | `lark_drive_permission_get_setting` | 读取支持类型的文件、文件夹或云文档自身权限设置，包括公开访问、分享、协作者管理、安全与评论 |
| `FACT_READ` | `lark_invoke(tool_name="lark_drive_file_statistics_get")` | 在用户要求活跃度、闲置暴露、生命周期或访问复核时读取文件访问统计 |
| `FACT_READ` | `lark_invoke(tool_name="lark_drive_file_view_records_list")` | 在用户要求最近访问人、访问复核或低活跃证据时读取访问记录 |
| `EXEC_CONFIRM` | `lark_drive_secure_label_list` | 提议 label update 前解析可用 secure-label IDs |
| `EXEC_CONFIRM` | `lark_invoke(tool_name="lark_drive_permission_members_auth")` | 目标公共访问和协作权限设置修改前检查 `action=manage_public` |
| `EXEC_CONFIRM` | `lark_discover(query="drive.permission.members.transfer_owner")` | owner 转移前读取当前字段、支持类型和高风险写入门禁 |
| `EXECUTE` | `lark_drive_apply_permission` | 向 owner 提交 view/edit access request；只允许单目标、小列表或已明确确认的候选列表逐个执行 |
| `EXECUTE` | `lark_invoke(tool_name="lark_drive_permission_public_patch")` | 修改已确认的 public/link settings |
| `EXECUTE` | `lark_invoke(tool_name="lark_drive_permission_members_transfer_owner")` | 转移已确认目标的 owner |
| `EXECUTE` | `lark_drive_secure_label_update` | 设置已确认的 secure-label ID |
| `VERIFY` | `lark_invoke(tool_name="lark_drive_metas_batch_query")`, `lark_drive_permission_get_setting` | 验证支持的 metadata，包括 owner、secure-label 和目标公共访问与协作权限设置变更；权限申请只能表述为已发起 |

## Command Patterns

本入口不内联命令样例。需要拼装具体命令时，按当前 state 调用 `lark_get_skill(domain="drive", section="workflow-permission-governance-commands")`。命令是否允许执行仍以 `Command Map` 和写入规则为准。
...
3. 权限设置读取使用 `lark_drive_permission_get_setting`，目标类型包括 `doc`、`sheet`、`file`、`wiki`、`bitable`、`docx`、`mindnote`、`minutes`、`slides`、`folder`、`apps`；未来新增类型以工具和 OpenAPI 元数据为准。
...
5. `folder` 作为递归容器时先枚举子资源；如用户明确要查询文件夹自身权限设置，可对该文件夹单独执行 `lark_drive_permission_get_setting(token="<folder_token>", type="folder")`。不要执行 `lark_invoke(tool_name="lark_drive_permission_public_patch")` 的 `type=folder`，除非 schema 和需求都明确支持。`shortcut`、`catalog` 或缺少 stable token/type 的条目必须记录为 unsupported，除非后续 API 明确解析出支持目标。
6. 对大范围目标输出进度时，只展示已扫描容器数、已发现目标数、已审计目标数、剩余队列或 blocker；不要默认展示内部 page token / cursor。

Wiki space / node 发现：

1. `/wiki/space/<space_id>` 直接解析为 `target_scope=wiki_space`。不要因为 `lark_drive_inspect` 对该 URL 返回 not found 就停止。
2. 用 `lark_wiki_node_list(space_id="<space_id>")` 读取根节点；当节点 `has_child=true` 时，用该节点的 `node_token` 继续递归读取子节点。
...
1. `/drive/folder/<folder_token>` 解析为 `target_scope=drive_folder`。默认继续枚举其子文档；只有用户明确要求文件夹自身权限设置时，才额外调用 `lark_drive_permission_get_setting(token="<folder_token>", type="folder")` 读取该文件夹自身设置。
2. 按 `lark_get_skill(domain="drive", section="files-list")` 递归处理 `data.files`、`has_more` 和 `next_page_token`。不要把第一页数量当作完整范围。
...
2. `lark_drive_permission_get_setting` 没有批量读取接口；对支持目标逐个读取。单个目标失败时记录 `unsupported_checks` 或 `partial`，不要阻断其他目标。
...
6. `permission_public` 用户可见含义是"目标公共访问和协作权限设置"，语义以官方 OpenAPI 字段说明为准，同时兼容当前工具 schema 返回的字段：优先使用 `external_access_entity`，缺失时才用 `external_access` boolean 映射为 `open` / `closed`；`manage_collaborator_entity`、`copy_entity`、`lock_switch` 等字段缺失时标记为 unknown，不要伪造；未识别字段保留在 raw evidence / partial note 中。
...
每个可审计目标都必须先归一化为 `per_target_permission_assessment`，再按 `lark_get_skill(domain="drive", section="workflow-permission-governance-outputs")` 的 `Semantic Rendering` 渲染。`public_exposure_check` 只是 `target_count=1` 的轻量渲染模式；它和多目标、容器诊断复用同一套语义字段与风险分类。该判断只覆盖当前目标公共访问和协作权限设置，不审计协作者名单、历史权限变更、完整继承链或审计日志。
...
- 权限设置读取使用 `lark_drive_permission_get_setting`；裸 token 必须传 `type`，URL 可以自动推断。写入仍使用 `lark_invoke(tool_name="lark_drive_permission_public_patch")`，只 patch 已解析且 schema 明确支持的类型和字段，不要把读取支持的 `folder` 自动外推为可写入。
- 不要 patch 已解析类型不支持的字段。对于 wiki 目标，必须省略 schema 明确标注为 wiki 不支持的字段。
- 不要在同一个写入确认中合并密级标签更新和目标公共访问与协作权限设置修改；必须分别确认。
- `lark_drive_apply_permission` 默认不批量执行；每次调用都会向 owner 发送通知。
...
- 容器范围内的"统一申请权限"必须先产出 `permission_request_candidates`。未展示候选目标、数量、权限类型和 owner 通知影响前，禁止调用 `lark_drive_apply_permission`。
- 用户显式确认批量权限申请后，也必须逐个目标顺序调用 `lark_drive_apply_permission`，并在结果中区分已发起申请、失败、无法构造申请请求和未发现目标。
...
- 针对 `selected_risk_items` 生成 dry-run 前，必须重新读取所选目标的 `lark_drive_permission_get_setting`；如果当前设置和清单快照不同，标记为 `changed_since_report` 并跳过或要求用户确认更新后的计划。
...
以下能力已有部分工具 surface 或用户价值，但不要在当前 workflow 中作为可执行分支直接调用：
...
- `lark_wiki_member_list` 可作为 Wiki space 成员治理的读侧事实来源；当前 workflow 只治理文档 / 节点 / 文件夹下可发现文档的权限，不做 space member governance。
- `lark_drive_member_list` 可读取单目标直接协作者/授权成员；当前工具仍没有完整继承链、DLP 扫描、AI 索引状态、审计日志和跨平台权限事实。遇到这些需求必须记录为 `unsupported_checks` 或建议新增独立 workflow。
...
- 面向用户优先使用业务语言和"候选风险 / 待复核 / 待策略确认"；底层字段只作为证据。完整模板按需调用 `lark_get_skill(domain="drive", section="workflow-permission-governance-outputs")`。
