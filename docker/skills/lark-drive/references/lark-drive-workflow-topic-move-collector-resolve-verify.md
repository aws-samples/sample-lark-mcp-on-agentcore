...
2. 按 `lark_get_skill(domain="drive", section="inspect")` 处理 URL / token 解析。
3. 使用 `lark_invoke(tool_name="lark_drive_metas_batch_query", ...)` 补齐 Drive 资源 owner、标题和 URL。
4. 必要时使用 `lark_invoke(tool_name="lark_drive_permission_members_auth", ...)` 读取权限信号；该接口不提供 `full_access` / 移动权限的直接判定，不能把 `manage_public` 等同为可移动。
5. 按 `lark_get_skill(domain="wiki", section="node-get")` 处理 Wiki 节点解析。
6. 按 `lark_get_skill(domain="doc", section="fetch")` 读取文档内容。
7. 需要验证 Sheet 内容时，按 `lark_get_skill(domain="sheets")` 执行。
...
| 候选类型 | agent 必须执行 |
|----------------|---------------|
| Drive URL / token | token 或类型不确定时，使用 `lark_drive_inspect`。 |
| Wiki URL / token | 使用 `lark_drive_inspect` 或 `lark_wiki_node_get`；保留节点身份和对象身份。 |
...
1. `lark_drive_permission_members_auth` 不提供 `full_access` 或 `move` action；不能用 `view`、`edit`、`share` 或 `manage_public` 结果推断源位置或目标位置可写。
...
3. 加载 `lark_get_skill(domain="drive", section="workflow-topic-move-collector-review-plan")`。
4. 不得直接进入 `PLAN_MOVE`。
...
| `owner_id` | 资源 owner；Drive 资源优先来自 `lark_invoke(tool_name="lark_drive_metas_batch_query", ...)`，Wiki 节点优先来自 `lark_wiki_node_get`。 |
