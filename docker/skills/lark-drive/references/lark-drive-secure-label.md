...
- `lark_drive_secure_label_list`：查询当前用户可用的密级标签，先拿到目标 `id`。
- `lark_drive_secure_label_update`：把目标云文档调整为指定密级标签。

这两个工具都使用用户身份。修改密级前，通常先执行 `lark_drive_secure_label_list` 确认可用标签 ID。
...
| 参数 | 说明 |
|------|------|
| `page_size` | 分页大小，范围 `1..10`，默认 `10` |
| `page_token` | 上一页响应里的 `page_token` |
| `lang` | 标签语言：`zh`、`en`、`ja` |
