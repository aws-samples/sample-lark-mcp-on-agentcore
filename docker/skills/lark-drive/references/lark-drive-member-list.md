...
读取 Drive 文档、文件、文件夹或 wiki 节点的协作者/授权成员列表。

## 命令

```
# URL 自动推断 type
lark_drive_member_list(token="https://example.feishu.cn/drive/folder/<folder_token>", format="json")

# 查询附加字段
lark_drive_member_list(token="<token>", type="docx", fields="name,type,external_label", format="json")
```
...
| `perm_type` | 否 | 仅 `type="wiki"` 有效；取值 `container` / `single_page`。 |
...
- **身份支持**：本工具通过 MCP server 始终以 user identity 执行；缺 scope 或目标权限时按统一 permission 错误路径处理。
...
- **folder 支持**：支持 `type="folder"` 并会按需求发送 `type=folder`；部分环境的后端如果尚未放开 folder 枚举，可能返回 `99992402 field validation failed`。
