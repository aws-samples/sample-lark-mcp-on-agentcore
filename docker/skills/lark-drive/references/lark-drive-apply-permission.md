...
传入 `/wiki/<node_token>` 时，shortcut 会直接用 `node_token` 作为路径参数并以 `type=wiki` 调用接口。如果需要先把 wiki 节点解析成 `obj_token`，自行先调用 `lark_wiki_node_get`（见 `lark_get_skill(domain="wiki", section="node-get")`）拿 `obj_token + obj_type`，再用 bare `obj_token` + `type="<obj_type>"` 调本命令。
