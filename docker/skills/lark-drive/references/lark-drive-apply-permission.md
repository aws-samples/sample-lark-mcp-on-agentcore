...
本工具对应：`lark_drive_apply_permission`。
...
- **仅支持 user identity**（使用 `user_access_token`），不支持 bot identity / `tenant_access_token`。
...
lark_drive_apply_permission(token="https://example.larksuite.com/docx/doxcnxxxxxxxxx", perm="view", remark="安全评估：需查看需求文档内容")

# 通过 bare token + 显式 type
lark_drive_apply_permission(token="doxcnxxxxxxxxx", type="docx", perm="edit")
```
...
API 成功时返回空 `data`（仅 `code: 0, msg: "success"`），对应输出：
...
| 错误码 | 含义 | 处理 |
|---|---|---|
| `1063006` | 申请次数已达上限（5 次/日） | 工具自动加 hint：`permission-apply quota reached: each user may request access on the same document at most 5 times per day` |
| `1063007` | 当前文档无法申请（如：文档禁用外部申请、申请者已拥有对应权限、目标类型不支持 apply） | 工具自动加 hint：`this document does not accept a permission-apply request ... contact the owner directly` |
...
传入 `/wiki/<node_token>` 时，shortcut 会直接用 `node_token` 作为路径参数并以 `type=wiki` 调用接口。如果需要先把 wiki 节点解析成 `obj_token`（例如想显式对底层 docx 申请），自行先调 `lark_invoke(tool_name="lark_wiki_spaces_get_node", args={params: {"token": "<node_token>"}})` 拿 `obj_token + obj_type`，再用 bare token + `type="docx"` 调本命令。
