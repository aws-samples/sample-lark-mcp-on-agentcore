...
1. 写操作确认、高风险操作、身份、认证和权限由 MCP server 自动处理。
2. 按 `lark_get_skill(domain="drive", section="create-folder")` 创建 Drive 文件夹。
3. 按 `lark_get_skill(domain="drive", section="move")` 执行 Drive 移动。
4. 按 `lark_get_skill(domain="wiki", section="node-create")` 创建 Wiki 节点。
5. 按 `lark_get_skill(domain="wiki", section="move")` 执行 Wiki 移动和 Drive 文档移动到 Wiki。
6. 按 `lark_get_skill(domain="wiki", section="move-to-drive")` 将 Wiki 节点移出到 Drive 文件夹。
7. 按 `lark_get_skill(domain="drive", section="delete")` 删除本次 workflow 新建的 Drive 文件夹。
8. 按 `lark_get_skill(domain="wiki", section="node-delete")` 删除本次 workflow 新建的 Wiki 节点。
9. 需要轮询异步任务时，按 `lark_get_skill(domain="drive", section="task-result")` 执行。
10. `MovePlanItem` schema 由 `lark_get_skill(domain="drive", section="workflow-topic-move-collector-review-plan")` 定义，本文件只消费已确认计划。
...
| 来源 -> 目标 | 移动方式 |
|------------------|-------------|
| Drive resource -> Drive folder | `lark_drive_move` |
| Drive document-like resource -> Wiki target | `lark_wiki_move` 的 docs-to-wiki 模式；默认不可自动恢复 |
| Wiki node -> Wiki target | `lark_wiki_move`（`node_token` 模式） |
| Wiki node -> Drive folder | `lark_wiki_move_to_drive` |

### 执行顺序

1. 如有 `create_target` 项，先执行。
2. 按确认计划顺序执行 `move_resource` 项。
3. 如果命令返回 task ID，用 `lark_drive_task_result` 执行异步任务轮询。
4. 输出写操作执行摘要。
...
7. 如果出现 `async_pending`，先使用 `lark_drive_task_result` 轮询确认；超过轮询限制后再报告 pending blocker。
...
8. 删除 workflow 新建的 Wiki 目标节点时，必须使用 `lark_wiki_node_delete(include_children=false, _confirm=true)`，让已迁入的直接子文档保留到该节点父级层级。
...
4. Wiki 新建目标清理使用 `include_children=false`；Drive 新建目标清理只在不会删除用户资源时执行。
...
| `resource_type` | `lark_drive_move` 恢复时必须传入的 `type`；非 Drive 恢复也保留原始资源类型。 |
...
```json
{
  ...
  "command_family": "lark_drive_move|lark_wiki_move|lark_wiki_move_to_drive|lark_drive_create_folder|lark_wiki_node_create|lark_drive_delete|lark_wiki_node_delete",
