...
`ResourceItem` schema 和字段生成规则由 `lark_get_skill(domain="drive", section="workflow-topic-move-collector-resolve-verify")` 负责。只要上述输入契约完整，本状态不得为重复读取 schema 而重新加载或执行前一阶段文档。
...
5. 不得把 `no_move_permission` 或 `move_permission_unknown` 资源直接提升到 `high` / `medium`；必须先回到 `RESOURCE_RESOLVE`，加载 `lark_get_skill(domain="drive", section="workflow-topic-move-collector-resolve-verify")` 取得可移动证据。
...
|------------------|---------------------------|
| `lark_drive_move` | `file_token`、`type`、`folder_token`；移动到 Drive root 时显式记录 `folder_token` 为空且目标类型为 root。 |
| `lark_wiki_move`（node） | `node_token`，以及 `target_space_id` 或 `target_parent_token`；可选 `source_space_id`。不得使用 `wiki_obj_token` 代替 `node_token`。 |
| `lark_wiki_move`（docs-to-wiki） | `obj_type`、`obj_token`、`target_space_id`、可选 `target_parent_token`，并显式保存 `apply=false`。 |
| `lark_wiki_move_to_drive` | `node_token`、`folder_token`；移动到 Drive root 时显式记录 `folder_token` 为空。 |
| `lark_drive_create_folder` | `name`、父级 `folder_token`；创建在 Drive root 时显式记录父级为空。 |
| `lark_wiki_node_create` | `space_id`、`title`、`obj_type`、可选 `parent_node_token`。 |
...
```json
{
  ...
  "command_family": "具体 shortcut 工具或 none",
  "command_args": {
