...
|-------|------|--------------|
| `lark-drive` SKILL | 用户意图到具体 workflow entry 的短路由 | 长流程逻辑、未注册场景 |
| 本总框架文件 | 共享运行协议、Artifact Contract、Workflow Registry、加载规则 | 非运行时背景说明、宽泛路线图、场景专项执行细节 |
| Registered workflow file | 场景范围、状态机、Command Map、确认门槛、验证规则 | 其他场景、隐藏写入、未被工具 / API 支持的能力声明 |
...
| `facts` | 从工具读取或引用 skill 获取的证据 |
| `plan_items` | 候选动作；每项包含 command family、target、risk、verification method |
| `unsupported_checks` | 因工具 / API 覆盖、目标类型、认证或范围限制而无法执行的检查 |
...
- 只有 workflow 已经具备可执行规则时，才允许作为本总框架 workflow 出现在 `lark-drive` SKILL 并加入 `Workflow Registry`。
- 多文件 phase 拆分只用于执行、回滚或验证流程复杂到影响可读性的 `S3` 场景。

## Workflow Registry

| Workflow | Status | Risk | Structure | Entry | Trigger |
|----------|--------|------|-----------|-------|---------|
| `permission_governance` | Registered | `R2` | `S2` | `lark_get_skill(domain="drive", section="workflow-permission-governance")` | 权限审计、公开链接/外部访问、复制/下载/评论/分享设置、权限申请、owner 转移 / 批量 owner 转移、密级标签调整 |
| `knowledge_organize` | Registered | `R2-R3` | `S3` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize")` | 整理云盘 / 文件夹 / 文档库 / 知识库、盘点目录结构、归类资源、生成整理方案，并在用户确认后创建目录或移动资源 |
| `topic_move_collector` | Registered | `R2-R3` | `S3` | `lark_get_skill(domain="drive", section="workflow-topic-move-collector")` | 按主题、关键词或内容线索跨容器搜索资料，验证相关性和移动资格，并在用户确认后归档到 Drive 文件夹或 Wiki 节点 |
...
2. 只在不新增本总框架 workflow 行为的前提下，将请求收窄为现有 skill / 工具可执行的原子操作。
