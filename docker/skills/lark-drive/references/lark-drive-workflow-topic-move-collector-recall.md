...
1. 身份、认证和权限由 MCP server 自动处理。
2. 按 `lark_get_skill(domain="drive", section="search")` 处理 `lark_drive_search` 语法、过滤条件、单批最多 5 页和身份语义；本 workflow 的全量续批规则见下文。

## 搜索原则

1. 默认使用 `lark_drive_search(mine=true)` 召回当前用户 owner / 负责的 Workspace 资源。
...
1. 用户确认进入 `topic_move_collector` 即表示同意为本次收集任务执行完整召回；无需再要求用户额外说“全部 / 全量 / 继续翻”。本规则覆盖 `lark_get_skill(domain="drive", section="search")` 的默认首屏交互规则。
2. 仍遵守 `lark_get_skill(domain="drive", section="search")` 的单轮最多 5 页限制。每读取最多 5 页形成一个批次；批次结束且 `has_more=true` 时，保存 checkpoint，并使用原 query、原过滤条件和返回的 `next_page_token` 自动开始下一批。
...
3. 不隐式添加 `folder_tokens` 或 `space_ids`。
4. 当 `owner_scope=mine` 时，所有基础 query 必须带 `mine=true`。
5. 当 `owner_scope=all_visible` 时，不带 `mine`，并记录扩展召回风险。
6. 除非命令限制要求更低值，否则使用 `page_size=20`。
...
3. 每个增强 query 都必须继承 `owner_scope`；`owner_scope=mine` 时必须带 `mine=true`。
...
3. 加载 `lark_get_skill(domain="drive", section="workflow-topic-move-collector-resolve-verify")`。
...
| `only_title` | 当标题命中更可信时使用。 |
| `only_comment` | 当主题可能只出现在评论讨论中时使用。 |
...
缺少认证 / scope、`lark_drive_search` 返回权限或策略阻塞、分页 token 无效、分页重试后仍无法继续，或工具预算不足以完成全部页面时，必须把对应 `QueryRecallState.status` 设置为 `blocked`，保留累计候选、页数和 `next_page_token`，停止并报告。阻塞解除后从 checkpoint 续跑；在全部 query 完成前不得进入资源解析或分类阶段。
