...
```
# 列出内置封面规格
lark_drive_cover(file_token="<FILE_TOKEN>", list_only=true)

# 下载 square 规格封面
lark_drive_cover(file_token="<FILE_TOKEN>", spec="square", output="./artifacts/report-cover")

# 下载默认大图封面，并在文件冲突时覆盖
lark_drive_cover(file_token="<FILE_TOKEN>", spec="default", output="./artifacts/report-cover.png", if_exists="overwrite")
```
...
- 不传 `list_only` 时，必须显式传 `spec` 和 `output`
- `lark_drive_cover` 只返回静态预设规格，不伪造后端"可下载状态"
