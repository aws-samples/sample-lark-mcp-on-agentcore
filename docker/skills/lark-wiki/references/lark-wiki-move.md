...
## 与 `lark_wiki_move_to_drive` / `lark_drive_move` 的区别

- `lark_wiki_move` 的目标是 **知识空间或 Wiki 父节点**，使用 `target_space_id` / `target_parent_token`
- `lark_wiki_move_to_drive` 把 **已有 Wiki 节点移出知识库，放入 Drive 文件夹或"我的空间"根目录**，使用 `folder_token`
- `lark_drive_move` 的目标是 **Drive 文件夹**，使用 `folder_token`
- 如果源对象已经是 Wiki 节点：目标仍是 Wiki 时使用 `lark_wiki_move`；目标是 Drive 文件夹或根目录时使用 `lark_wiki_move_to_drive`
...
- 当用户说"移动到某个文件夹""移动到云空间（云盘/云存储）根目录"时，按 **Drive 文件夹目标** 处理；源对象是 Wiki 节点时使用 `lark_wiki_move_to_drive`，源对象已在 Drive 时使用 `lark_drive_move`
...
- 调用 `lark_get_skill(domain="wiki", section="move-to-drive")` 查看将 Wiki 节点移出知识库并放入 Drive
