...
- **同步 / 异步两种返回**：
  ...
  - 如果接口返回非空 `task_id`，shortcut 会先对任务做有限轮询；轮询窗口内仍未完成会输出 `next_command`，引导调用方使用 `lark_drive_task_result(scenario="wiki_delete_space", task_id="<TASK_ID>")` 继续查

## 命令

```
# 同步或异步删除一个知识空间（高风险写操作，需要 _confirm=true）
...
| `_confirm` | 是（真删时） | 高风险写操作确认。不传则工具直接返回 `unsafe_operation_blocked` 错误 |
...
- **继续查询**：看到 `next_command` 后，改用 `lark_drive_task_result(scenario="wiki_delete_space", task_id="<TASK_ID>")` 继续查
...
```json
{
  ...
  "timed_out": true,
  "next_command": "lark_drive_task_result(scenario=\"wiki_delete_space\", task_id=\"7631425120875056669-965458aec67417f5982250806c97950697ccb82f\")"
}
```
...
当前 shortcut 声明的权限为 `wiki:space:write_only` 和 `wiki:space:read`。前者用于发起删除请求，后者用于轮询同一命令内的异步任务状态。

异步超时后的 `lark_drive_task_result(scenario="wiki_delete_space", task_id="<TASK_ID>")` 只需 `wiki:space:read`（纯读任务状态）。

## 空间解析：如何拿到 `space_id`

`lark_wiki_delete_space` 只接受 `space_id` 作为目标。用户在对话里常常只说知识库的**名称**或贴一条**知识库 URL**，这时**不能**把名称 / URL 原样当成 `space_id` 传进去，必须先解析。三种输入路径：
...
调用 `lark_wiki_space_list`：

```
# 第一页
lark_wiki_space_list(format="json")

# 如果需要继续翻页（看下方停止条件），带上 page_token
lark_wiki_space_list(page_token="<上一页返回的 page_token>", format="json")
```
...
```
lark_wiki_delete_space(space_id="<RESOLVED_SPACE_ID>", _confirm=true)
```
...
- 框架会强制要求 `_confirm=true` 确认；不传 `_confirm` 时工具会直接返回 `unsafe_operation_blocked` 错误，不会真的发请求

> [!CAUTION]
> `lark_wiki_delete_space` 是**不可逆的写入操作**。执行前务必与用户再次确认 `space_id`，并清楚该空间下的所有节点都会一并被删除。

## 参考

- 调用 `lark_get_skill(domain="wiki")` 查看知识库全部命令
- 调用 `lark_get_skill(domain="drive", section="task-result")` 查看异步任务的续跑查询命令
