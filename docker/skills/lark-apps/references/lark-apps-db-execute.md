...
用于通过妙搭服务端执行应用数据库 SQL。不要从环境变量里取连接串裸连数据库；本地调试也走这个工具。写什么样的 SQL（平台约束、建表模板、`user_profile`、审计列、禁用 SQL、PG 陷阱）见文末「平台 SQL 规范」。
...
上面讲工具怎么调，这里讲**该写出什么样的 SQL**：妙搭底层是 PostgreSQL + 一层平台约束（RLS、审计列、`user_profile` 复合类型、禁用 SQL 白名单），不符合会被服务端直接拒或建出行为不对的表。看表 / 看结构用 `lark_apps_db_table_list` / `lark_apps_db_table_get`（见 `lark_get_skill(domain="apps", section="db")`），别手写系统表查询模拟。
...
新建业务表必须：4 个审计列 + 启用 RLS + 4 条默认 policy，**放在同一次 `lark_apps_db_execute` 调用里**（RLS / policy / COMMENT / INDEX 一起）。裸表名，不写 `public.` 或 schema 前缀。
...
建表流程：先 `lark_apps_db_table_list` / `lark_apps_db_table_get` 确认表不存在或看现有结构 → 生成 DDL → 向用户展示影响并取得授权 → `lark_apps_db_execute(..., _confirm=true)` 执行。
...
**多环境库加约束前先查 online 存量**：`dev` 干净不代表 `online` 干净，约束发布到 online 会撞线上存量数据而失败。发布前一律先查 `environment="online"` 清楚，按约束类型分三种：

- **加唯一约束（`UNIQUE` / 唯一索引）**：线上不能有重复值。先查重复，有则先清理再加：

  ```
  lark_apps_db_execute(app_id="app_xxx", environment="online", sql="SELECT <cols>, count(*) FROM t GROUP BY <cols> HAVING count(*) > 1", _confirm=true)
  ```

- **已有列改 `NOT NULL`（收紧约束）**：线上该列不能有 NULL。先查 NULL 行数，有就先回填（`UPDATE t SET <col> = <默认值> WHERE <col> IS NULL`）再加约束：

  ```
  lark_apps_db_execute(app_id="app_xxx", environment="online", sql="SELECT count(*) FROM t WHERE <col> IS NULL", _confirm=true)
  ```
...
- **新加 `NOT NULL` 字段**：必须带 `DEFAULT`，且要求线上该表**无存量数据**，否则发布报错。线上已有数据时别直接加，改走三步安全变更：先 `ADD COLUMN <col> <type>`（可空）→ 回填 `UPDATE t SET <col> = <值>` → 再 `ALTER COLUMN <col> SET NOT NULL`。先查线上行数判断走哪条：
  ...
  ```
  lark_apps_db_execute(app_id="app_xxx", environment="online", sql="SELECT count(*) FROM t", _confirm=true)
  ```
...
- 已有表 / 已有数据默认禁止；先 `SELECT count(*)` 展示命中行数、取得用户明确授权，再带 `_confirm=true` 执行。
...
| 手写系统表查结构 | 常规结构查询用 `lark_apps_db_table_list` / `lark_apps_db_table_get`，别手写 `information_schema` / `pg_indexes` 模拟 |
...
| MySQL 方言 | 不用 `SHOW TABLES` / `DESCRIBE` / 内联 `COMMENT`；用 `lark_apps_db_table_*` 和 `COMMENT ON` |
