...
纯前端应用（vite-react，无数据库）。流程与 full_stack 基本一致——`lark_apps_init` 装依赖、`npm run dev`、commit/push/release——差别是无 `lark_apps_db_*` 调库步骤。后续需要数据库/后端能力时不在本地升级，按 SKILL.md「类型升级」引导到云端会话。
...
# 初始化本地仓库（dir 取值见下方「领域规则」，勿照抄此处示例值）
lark_apps_init(app_id="app_xxx", dir="./json-tool")
```
...
```
lark_apps_release_create(app_id="app_xxx", branch="sprint/default")
# 发布是异步的：用 lark_apps_release_get 轮询到 status=finished 才算部署完成、拿到 online_url
lark_apps_release_get(app_id="app_xxx", release_id="<上一步返回的 release_id>")
```
