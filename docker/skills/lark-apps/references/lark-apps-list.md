...
- `app_type` 枚举：`html` / `frontend` / `full_stack`。
- 分页：`page_size` 默认 20，`page_token` 传上一页 cursor。

## 示例

```
lark_apps_list(keyword="审批")
lark_apps_list(ownership="mine", app_type="full_stack")
lark_apps_list(page_token="<cursor>")
```
...
- 默认输出已裁掉 `icon_url`（图片 URL，agent 无法渲染）和 `created_at`（与 `updated_at` 冗余）。
