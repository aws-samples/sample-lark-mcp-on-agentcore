...
- **严禁把 `approvals get` 返回的 `form` 当成可直接提交的原样模板。**
- **严禁在未得到用户确认前直接执行真实提单。**
...
- 值本身从哪里拿，优先按 `lark_get_skill(domain="approval", section="instance-value-sourcing")` 处理；不要把“知道结构”误当成“已经拿到可提交值”。
- 若 `lark_get_skill(domain="approval", section="instance-form-control-parameters")` 标明某控件不支持通过创建实例 API 提交，则不要硬猜绕过；应明确告诉用户该定义当前无法仅通过 API 提单。
- 若遇到当前 skill 未明确覆盖的复杂控件，不要硬猜；先依据 `lark_get_skill(domain="approval", section="instance-form-control-parameters")` 判断支持性与传值结构，再向用户确认。

## API 不支持的控件

根据 `lark_get_skill(domain="approval", section="instance-form-control-parameters")`，创建审批实例 API 不支持的控件至少包括：
...
优先按 `lark_get_skill(domain="approval", section="instance-form-control-parameters")` 组装，下面只保留最常用、最容易出错的格式：
...
`lark_get_skill(domain="approval", section="instance-form-control-parameters")` 还明确给出了若干特殊控件组的提单格式，至少包括：
...
- 先从 `approvals get` 返回的 `form` 找到控件组及其子控件 ID
...
5. `approvals get` 返回的 `node_list`：提供节点 key 与是否需要补充审批人/抄送人的线索。

不要反过来把 `approvals get` 返回的 `form` 当成第一优先级，更不要把它当成可直接提交的 JSON 模板。
