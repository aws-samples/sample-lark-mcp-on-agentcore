...
**判定规则：** 只要最终动作是对审批单据做同意、拒绝、转交、退回、撤回、催办、加签、抄送、查详情、查已发起/已办/待办，就归 approval 域。只有当用户处理的是**非审批类任务/待办**时，才走 `lark_get_skill(domain="task")`。
...
lark_invoke(tool_name="lark_approval_instances_create", args={data: {"approval_code":"<code>","form":"[...]"}, _confirm: true})
