...
- 用户只给出**姓名**时，先用 `lark_get_skill(domain="contact")` 里的方式把姓名解析成 open_id，再填入 `mentionUserId`。
...
写入画板走标准 DSL 路径（DSL 产物先用 `npx -y @larksuite/whiteboard-cli@^0.2.13 -i diagram.json --to openapi --format json` 转换，再用 `lark_whiteboard_update(input_format="raw", ...)` 写入），无需手写 raw JSON。
...
- **用假 id 或用户中文名当 id**：`mentionUserId` 只接受真实 open_id，先用 `lark_contact_search_user` 解析。
