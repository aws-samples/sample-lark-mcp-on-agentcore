...
| 获取画板的 Mermaid/PlantUML 代码 | `lark_whiteboard_export(whiteboard_token="xxx", output_type="source")` — 详见 `lark_get_skill(domain="whiteboard", section="export")` |
| 检查画板是否由代码绘制 | `lark_whiteboard_export(whiteboard_token="xxx", output_type="source")` — 详见 `lark_get_skill(domain="whiteboard", section="export")` |
| 仅微调节点文字/颜色 | `lark_whiteboard_export(output_type="raw")` → 手动改 JSON → `lark_whiteboard_update(input_format="raw")` |
