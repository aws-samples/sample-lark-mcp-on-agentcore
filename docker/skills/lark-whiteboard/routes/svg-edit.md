...
- **未确认或回复含糊** → 原样向用户发出下面这句话，**然后立即结束本回合等待回复** —— 同一条消息内不得附带任何导出/编辑/写回的工具调用：

> SVG 编辑只保证视觉层面对齐，画板语义（层级/节点类型/思维导图结构/表格结构/连线绑定/容器类型/mention 等）将不可恢复，是否继续？

这是**知情确认**（动手前让用户对语义丢失止损）；真正的破坏性写入在 Step 4 还会再经 `overwrite` + `_confirm` 确认一次，二者职责不同、都不可省。
...
在导出的 SVG 上进行修改。参考 `lark_get_skill(domain="whiteboard", section="routes/svg")` 的「画板怎么处理 SVG」章节，了解可识别元素与不支持的装饰特性。
...
`overwrite=true` 会清空原画板内容，确认后再执行

```
lark_whiteboard_update(whiteboard_token="<TOKEN>", source="@<dir>/edited.svg", input_format="svg", idempotent_token="<10+字符唯一串>", overwrite=true, _confirm=true)
```
