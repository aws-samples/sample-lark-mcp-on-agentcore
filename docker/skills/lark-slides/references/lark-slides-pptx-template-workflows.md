...
直接使用以下工具，不需要先加载 `lark-drive` skill：

```
lark_drive_import(file="<template.pptx>", type="slides")
```

可选参数：用 `name="<title>"` 指定导入后的 Slides 标题；用 `folder_token="<FOLDER_TOKEN>"` 指定目标文件夹。若返回 `ready=false` / `timed_out=true`，直接按返回值里的 `next_command` 继续轮询；等价形式是：
