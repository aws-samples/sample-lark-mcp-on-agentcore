...
```
lark_exec_script(script="lark-slides/scripts/iconpark_tool.py", args=["search", "--query", "增长趋势", "--limit", "8"])
lark_exec_script(script="lark-slides/scripts/iconpark_tool.py", args=["resolve", "--name", "chart-line"])
lark_exec_script(script="lark-slides/scripts/iconpark_tool.py", args=["list-categories"])
```
...
- 默认先检索：语义图标需求必须先用 `lark_exec_script` 调用 `iconpark_tool.py search --limit 8` 或 `--limit 10`，让 agent 从候选里结合版面语义二次判断；不要编造不存在的 `iconType`。
