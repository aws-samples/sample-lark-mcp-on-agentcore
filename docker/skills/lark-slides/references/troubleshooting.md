...
1. 先判断是否已有可用的 `xml_presentation_id`：从成功返回、错误 hint、用户给定链接或已保存上下文中获取；没有 ID 时不要回读，直接按当前错误处理。
2. 如果有 `xml_presentation_id`，再用 `lark_slides_xml_get` 尝试回读，确认是否存在演示文稿、是否已有部分页面写入、或是否只是空 presentation。
3. 检查失败页是否含未转义字符：`Q&A -> Q&amp;A`，文本 `<` / `>` 写成 `&lt;` / `&gt;`，属性 URL `a=1&b=2 -> a=1&amp;b=2`。
4. 检查标签闭合、属性引号、`<content>` 结构，以及 `<slide>` 直接子元素。
5. 如果使用 `slides` 参数一次性建多页失败，直接切到两步创建：先 `lark_slides_create`，再用 `lark_invoke(tool_name="lark_slides_xml_presentation_slide_create", ...)` 逐页添加。
...
| 图片不显示 / `<img src>` 仍是 `@path` | `@` 占位符只在 `lark_slides_create` 的 `slides` 参数中替换；直接调 `xml_presentation.slide create` 必须先用 `lark_slides_media_upload` 拿 `file_token` |
...
| 3350002 | `revision_id` 大于当前版本 | 用 `-1` 取当前版本，或重新用 `lark_slides_xml_get` 取最新 `revision_id` |
| validation: unsafe file path | `file` 参数给了绝对路径或上层路径 | `file` 必须是工作目录内的相对路径 |
