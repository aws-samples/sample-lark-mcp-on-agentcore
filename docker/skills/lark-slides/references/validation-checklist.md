...
2. 用 `lark_slides_xml_get` 回读全文 XML 到本地文件。
3. 检查实际页数是否符合计划或用户要求。
...
```
lark_exec_script(script="lark-slides/scripts/xml_text_overlap_lint.py", args=["--input", "-"], stdin="<待检查的 presentation 或 slide XML>")
```
...
| `iconpark_unsupported_icon_type` | `<icon>` 使用了 `iconpark-index.json` 中不存在的 `iconType` | 按 lint `hint` 改为名单内的 `iconType`，或先用 `lark_exec_script(script="lark-slides/scripts/iconpark_tool.py", ...)` 搜索 |
...
```text
验证记录：
- 回读：已执行 lark_slides_xml_get，实际页数 N / 预期 N。
