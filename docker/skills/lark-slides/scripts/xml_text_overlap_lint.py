...
import re
import sys
# nosemgrep: python.lang.security.use-defused-xml.use-defused-xml -- XXE/XML-bomb blocked before parsing by ensure_safe_xml_source() (rejects DOCTYPE/ENTITY, caps size); container has no pip for defusedxml
...
# and cap input size before ET.fromstring. Standard-library only — the container
# runs these skill scripts with the base image's python3 (no pip / third-party
# deps), so defusedxml is intentionally not used. Mirrors doc_word_stat.py.
...
def run_cli(argv: list[str] | None = None) -> None:
    ...
    if options["input"] == "-":
        ...
        result = lint_xml(xml_content, "<stdin>")
    else:
