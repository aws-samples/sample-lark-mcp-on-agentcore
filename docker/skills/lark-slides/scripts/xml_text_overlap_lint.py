...
import xml.etree.ElementTree as ET  # nosemgrep: python.lang.security.use-defused-xml.use-defused-xml -- XXE/XML-bomb blocked before parsing by ensure_safe_xml_source() (rejects DOCTYPE/ENTITY, caps size); container has no pip for defusedxml
...
# XXE / XML-bomb defense for the stdlib parser: reject DOCTYPE/ENTITY declarations
# and cap input size before ET.fromstring / expat. Standard-library only — the
# container runs these skill scripts with the base image's python3 (no pip /
# third-party deps), so defusedxml is intentionally not used. Mirrors doc_word_stat.py.
...
def ensure_safe_xml_source(source: str) -> None:
    if len(source) > MAX_XML_INPUT_CHARS:
        fail(f"XML input is too large ({len(source)} chars, limit {MAX_XML_INPUT_CHARS})")
    if FORBIDDEN_XML_DECL_RE.search(source):
        fail("XML input must not contain DOCTYPE or ENTITY declarations")
...
def run_cli(argv: list[str] | None = None) -> None:
    ...
    resolved_path = "-" if requested_path == "-" else str(Path(requested_path).resolve())
