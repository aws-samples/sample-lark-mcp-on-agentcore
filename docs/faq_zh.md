...
**Q: Quick Desktop 连接时认证失败（错误码 20029）？**

A: 检查部署输出的 Redirect URL 是否注册到了正确的开发者后台：
...
A: 检查两点：1) 部署输出的 Redirect URL 是否注册到了正确的开发者后台（飞书：open.feishu.cn / Lark：open.larksuite.com，须与 `LARKSUITE_CLI_BRAND` 一致）；2) Client Secret 是否与部署输出一致（如不确定可运行 `./scripts/ops.sh rotate-secret` 重新生成）。
...
A: 支持。运行 `./scripts/deploy.sh` 时会出现版本选择器——选择 **Lark
(international)**，OAuth 授权流程（`*.larksuite.com`）与 `lark-cli` 的 API 网关会
同时切到国际版集群。非交互 / `--yes` 部署时，在环境变量中设置 `LARK_BRAND=lark`
（或 `LARKSUITE_CLI_BRAND=lark`）。该选择会记录在 `deploy-config` 中。默认是
**Feishu（中国）**（`*.feishu.cn`）。请在 `open.larksuite.com`（而非
`open.feishu.cn`）获取并填写你的 Lark 应用凭证。
