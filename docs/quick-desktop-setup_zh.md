...
| Client Secret | 来自部署输出（保存在 `.local/deploy-output.md`），或通过以下命令获取：<br>`aws ssm get-parameter --name /lark-mcp-on-agentcore/oauth-client-secret --with-decryption --query 'Parameter.Value' --output text --region <your-region>` |
| Token URL | `<OAuth 端点>/token` |
| Authorization URL | `<OAuth 端点>/authorize` |
...
连接成功后，即可在 Quick Desktop 里向其内置 AI 提需求来操作飞书（自然语言理解和编排由 Quick Desktop 自带模型完成，本服务负责调用飞书 API）：
